...
export function getEffectiveModel(request) {
  return getEffectiveModelName(request);
}
...
/**
 * SVG-aware wrapper around the session-scoped raw identity resolver.
 *
 * @returns {Object|null} modelInfo (svg/color/name/...) 或 null（caller 应渲染中性 'MainAgent' 头像）
 */
export function resolveProducerModelInfo(options) {
  const name = resolveProducerModelName(options);
  ...
  for (const provider of MODEL_PROVIDERS) {
    if (provider.match.test(modelName)) {
      info = {
        ...
        // OpenAI's logo is monochrome in the product UI.  Replace the source
        // asset's legacy brand fill so every inline-SVG consumer inherits
        // --model-logo-mono (dark: white, light: black) through currentColor.
        svg: provider.monochrome
          ? provider.svg.replace(/\sfill="[^"]*"/g, ' fill="currentColor"')
          : provider.svg,
        svgAnimated: provider.svgAnimated || null,
        monochrome: provider.monochrome === true,
      };
      break;
    }
  }
