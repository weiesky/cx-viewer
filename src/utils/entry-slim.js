...
// pool 容量上限防御异常输入；正常 cx-viewer session 内 tools/system 配置稳定，
...
function _systemSig(system) {
  if (Array.isArray(system)) {
    ...
    for (const b of system) {
      const text = b?.text || '';
      // 边界增强（CR P1 defensive）：cx-viewer 场景中 system block 跨 entry 通常同模板，
...
function _internOrAdd(pool, sig, value) {
  ...
  if (pool.size >= _MAX_INTERN_POOL_SIZE) {
    // FIFO eviction — 早期插入的优先丢；cx-viewer 场景 tools/system 配置稳定，pool
