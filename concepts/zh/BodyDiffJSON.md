...
Codex 的 MainAgent 采用全量上下文发送机制。对于 OpenAI Responses API，每次请求都会携带完整的 `input`、`instructions`、工具定义等内容。这意味着随着对话推进，请求体会越来越庞大，直接查看原始 Body 很难快速定位"这一轮到底新增了什么"。
...
3. **智能差异提取**：
   ...
   - 删除的字段：不显示（通常不影响理解）
   - 变更的字段：展示当前值
   - `input` 数组特殊处理：只展示新增的输入项（因为正常对话是追加模式，前缀 input 不变）
4. **请求体缩小检测**：如果当前请求体比上一个更小，说明发生了上下文截断或会话重置，此时会显示提示信息而非 diff

## 典型场景

在一轮正常对话中，Body Diff JSON 通常只包含：
- `input`：新增的 1~2 个输入项（用户的输入 + 上一轮助手的回复）

如果你看到 diff 中出现了 `instructions`、`tools`、`model` 等字段的变更，说明本轮请求发生了配置变化，这往往也是缓存重建的原因。
...
- 在左上角 **CX-Viewer → 全局设置** 中，可以设置"默认展开 Body Diff JSON"
