...
MainAgent é a cadeia de requisições principal do Codex quando não está no modo agent team. Cada interação do usuário com o Codex gera uma série de requisições à API, e as requisições do MainAgent formam a cadeia de diálogo central — elas carregam o system prompt completo, definições de ferramentas e histórico de mensagens.
...
- O system prompt contém texto característico de "Codex"

## Diferença em Relação ao SubAgent

| Característica | MainAgent | SubAgent |
|------|-----------|----------|
| system prompt | Prompt principal completo do Codex | Prompt simplificado específico para a tarefa |
