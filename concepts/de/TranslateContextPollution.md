...
| OAuth-Token | `authorization: Bearer sessionToken` | Codex-Abonnement-Login | Sitzungsgebunden, Server pflegt Kontextzuordnung |
...
Wenn Codex die OAuth-Anmeldung über ein Abonnement verwendet, sieht der Authentifizierungsfluss folgendermaßen aus:

```
Codex Hauptkonversation ──(authorization: Bearer sessionToken)──→ Anthropic API
                                                                          ↑
CC-Viewer Übersetzungsanfrage ──(authorization: Bearer sessionToken)──→ Anthropic API
```

Da Übersetzungsanfragen dasselbe Sitzungstoken wiederverwendeten, konnte der Anthropic-Server Übersetzungsanfragen mit dem Hauptkonversationskontext von Codex verknüpfen. Dies führt zu:

1. **Übersetzungsergebnisse werden vom Hauptkonversationskontext beeinflusst**: Der Systemprompt der Übersetzungsanfrage lautet „Du bist ein Übersetzer", aber der Serverkontext enthält weiterhin den Gesprächsverlauf von Codex, was das Modell beeinträchtigen kann
2. **Hauptkonversation wird durch Übersetzungsanfragen gestört**: Inhalte der Übersetzungsanfrage (UI-Textfragmente) können in den Hauptkonversationskontext injiziert werden, wodurch die Antworten von Codex abweichen
