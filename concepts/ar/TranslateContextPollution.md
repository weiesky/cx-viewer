...
يتضمن CC-Viewer ميزة ترجمة مدمجة (`POST /api/translate`) تعمل بواسطة Anthropic Messages API. في التنفيذ المبكر، كانت طلبات الترجمة تعيد استخدام بيانات المصادقة المخزنة مؤقتاً من جلسة Codex — بما في ذلك رؤوس `x-api-key` و `authorization`. تسبب هذا في مشكلة خفية لكنها خطيرة: كانت نتائج الترجمة تعيد في كثير من الأحيان محتوى غير ذي صلة.
...
| رمز OAuth | `authorization: Bearer sessionToken` | تسجيل دخول اشتراك Codex | مرتبط بالجلسة، يحتفظ الخادم بارتباط السياق |
...
عندما يستخدم Codex تسجيل دخول OAuth بالاشتراك، يبدو تدفق المصادقة كالتالي:

```
المحادثة الرئيسية لـ Codex ──(authorization: Bearer sessionToken)──→ Anthropic API
                                                                              ↑
طلب ترجمة CC-Viewer ──(authorization: Bearer sessionToken)──→ Anthropic API
```

نظراً لأن طلبات الترجمة أعادت استخدام نفس رمز الجلسة، فقد يربط خادم Anthropic طلبات الترجمة بسياق المحادثة الرئيسية لـ Codex. يؤدي هذا إلى:

1. **تأثر نتائج الترجمة بسياق المحادثة الرئيسية**: موجه النظام لطلب الترجمة هو "أنت مترجم"، لكن سياق الخادم لا يزال يحتوي على سجل محادثات Codex، مما قد يتداخل مع النموذج
2. **تعطل المحادثة الرئيسية بسبب طلبات الترجمة**: قد يتم حقن محتوى طلبات الترجمة (أجزاء نصوص واجهة المستخدم) في سياق المحادثة الرئيسية، مما يتسبب في انحراف استجابات Codex
