...
CC-Viewer มีฟีเจอร์แปลภาษาในตัว (`POST /api/translate`) ที่ขับเคลื่อนโดย Anthropic Messages API ในการพัฒนาช่วงแรก คำขอแปลภาษาใช้ข้อมูลรับรองตัวตนที่แคชไว้จากเซสชัน Codex ซ้ำ — รวมถึงทั้ง header `x-api-key` และ `authorization` สิ่งนี้ทำให้เกิดปัญหาที่แนบเนียนแต่ร้ายแรง: ผลลัพธ์การแปลมักส่งคืนเนื้อหาที่ไม่เกี่ยวข้อง
...
| OAuth Token | `authorization: Bearer sessionToken` | การเข้าสู่ระบบแบบสมัครสมาชิก Codex | ผูกกับเซสชัน เซิร์ฟเวอร์รักษาการเชื่อมโยงบริบท |
...
เมื่อ Codex ใช้การเข้าสู่ระบบ OAuth แบบสมัครสมาชิก กระบวนการรับรองตัวตนจะเป็นดังนี้:

```
การสนทนาหลักของ Codex ──(authorization: Bearer sessionToken)──→ Anthropic API
                                                                          ↑
คำขอแปลภาษาของ CC-Viewer ──(authorization: Bearer sessionToken)──→ Anthropic API
```

เนื่องจากคำขอแปลภาษาใช้ session token เดียวกันซ้ำ เซิร์ฟเวอร์ Anthropic อาจเชื่อมโยงคำขอแปลภาษาเข้ากับบริบทการสนทนาหลักของ Codex ส่งผลให้:

1. **ผลลัพธ์การแปลได้รับอิทธิพลจากบริบทการสนทนาหลัก**: system prompt ของคำขอแปลคือ "คุณคือนักแปล" แต่บริบทของเซิร์ฟเวอร์ยังคงมีประวัติการสนทนาของ Codex ซึ่งอาจรบกวนโมเดล
2. **การสนทนาหลักถูกรบกวนจากคำขอแปลภาษา**: เนื้อหาของคำขอแปล (ส่วนข้อความ UI) อาจถูกแทรกเข้าไปในบริบทการสนทนาหลัก ทำให้คำตอบของ Codex เบี่ยงเบน
