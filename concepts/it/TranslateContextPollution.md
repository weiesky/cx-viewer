...
CC-Viewer include una funzionalità di traduzione integrata (`POST /api/translate`) basata sull'API Messages di Anthropic. Nell'implementazione iniziale, le richieste di traduzione riutilizzavano le credenziali di autenticazione memorizzate nella cache della sessione di Codex — inclusi gli header `x-api-key` e `authorization`. Questo ha causato un problema sottile ma grave: i risultati delle traduzioni restituivano frequentemente contenuti irrilevanti.
...
| Token OAuth | `authorization: Bearer sessionToken` | Login con abbonamento Codex | Legato alla sessione, il server mantiene l'associazione del contesto |
...
Quando Codex utilizza il login OAuth tramite abbonamento, il flusso di autenticazione si presenta così:

```
Conversazione principale Codex ──(authorization: Bearer sessionToken)──→ Anthropic API
                                                                                 ↑
Richiesta di traduzione CC-Viewer ──(authorization: Bearer sessionToken)──→ Anthropic API
```

Poiché le richieste di traduzione riutilizzavano lo stesso token di sessione, il server di Anthropic poteva associare le richieste di traduzione al contesto della conversazione principale di Codex. Questo causa:

1. **I risultati della traduzione sono influenzati dal contesto della conversazione principale**: Il prompt di sistema della richiesta di traduzione è "sei un traduttore", ma il contesto del server contiene ancora la cronologia della conversazione di Codex, potenzialmente interferendo con il modello
2. **La conversazione principale viene disturbata dalle richieste di traduzione**: Il contenuto delle richieste di traduzione (frammenti di testo dell'interfaccia) può essere iniettato nel contesto della conversazione principale, causando deviazioni nelle risposte di Codex
