...
Codex의 MainAgent는 전체 컨텍스트 전송 방식을 채택하고 있어, 매 요청마다 완전한 대화 이력, system prompt, 도구 정의 등의 내용을 포함합니다. 이는 대화가 진행될수록 요청 본문이 점점 커진다는 것을 의미하며, 원시 Body를 직접 확인해서는 "이번에 실제로 무엇이 추가되었는지"를 빠르게 파악하기 어렵습니다.
