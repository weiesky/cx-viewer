...
import { getResponseTools } from './openai-body.js';
...
export function createRequestSpan(requestEntry) {
  ...
  const span = _tracer.startSpan(spanName, {
    ...
    attributes: {
      ...
      ...(body?.input && { 'cx.codex.input_count': body.input.length }),
      ...(getResponseTools(body).length > 0 && { 'cx.codex.tool_count': getResponseTools(body).length }),
    },
  });
