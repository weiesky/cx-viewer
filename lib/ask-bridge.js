#!/usr/bin/env node
/**
 * ask-bridge.js — PreToolUse hook bridge for request_user_input.
 *
 * Called by Codex when request_user_input is about to execute.
 ...
 *
 * Hook config in ~/.codex/hooks.json:
 * {
 *   "hooks": {
 *     "PreToolUse": [{
...
if (!port) {
  ...
  // exit(1) causes Codex to log "hook error" on every request_user_input call
