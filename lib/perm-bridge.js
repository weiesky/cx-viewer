 ...
 *
 * Registered with matcher: "" (empty = match all tools) in ~/.codex/settings.json.
 * Called by Codex before any tool executes. Reads hook payload from stdin,
 * forwards to cx-viewer server via long-poll HTTP, waits for user decision
...
if (!port) {
  ...
  // exit(1) causes Codex to log "hook error" on every tool call
...
if (process.env.CXV_BYPASS_PERMISSIONS === '1' && !isPublishCmd) {
...
try {
  ...
  if (decision === 'deny') {
    output.hookSpecificOutput.permissionDecisionReason = 'User denied via cx-viewer';
  }
  ...
  // continue: true means "this hook has no decision, let Codex handle it normally"
