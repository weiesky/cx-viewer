 ...
 *
 * Wraps @openai/codex query() to provide:
 ...
 * - Completed-entry fallback when a turn only exposes final assistant text via stream events
 */
...
let _pendingFinalContent = null;
let _pendingFinalRequestMessages = null;
...
async function _executeQuery(text) {
  ...
  _pendingFinalContent = null;
  _pendingFinalRequestMessages = null;
...
function _processMessage(msg) {
  switch (msg.type) {
    ...
    case 'result':
      if (!_finalEntryEmittedThisTurn && _pendingFinalContent?.length > 0) {
        _emitCompletedEntryFromStream();
      }
      if (msg.session_id) _sessionId = msg.session_id;
      if (_onStreamingStatus) _onStreamingStatus(buildStreamingStatus(false));
      break;
...
function _processStreamEvent(event) {
  ...
  if (type === 'message_start') {
    ...
    if (_streamingContent.length > 0) {
      _pendingFinalContent = [..._streamingContent];
      _pendingFinalRequestMessages = [..._accumulatedMessages];
    }
    _resetStreamingState();
  }
}
...
function _emitCompletedEntryFromStream() {
  if (!_onEntry || !_pendingFinalContent?.length) return;

  const requestMessages = _pendingFinalRequestMessages || [..._accumulatedMessages];
  const content = _filterInteractiveContent(_pendingFinalContent);
  const syntheticMsg = {
    message: {
      id: _streamingRequestId,
      type: 'message',
  ...
  const entry = sdkToJSONLEntry(syntheticMsg, requestMessages, _model, _projectName, {
    timestamp: _turnTimestamp,
    tools: _sdkTools,
  });
  _onEntry(entry);
...
async function _handleCanUseTool(toolName, input, options) {
  ...
  if (decision === 'deny') {
    return { behavior: 'deny', message: 'User denied via cx-viewer' };
  }
