 ...
 * - 去重：通常用 regex 提取 key；仅 Models 基准/重复标记做 JSON.parse 还原
 * - 异步发送：逐条 write + 定期 setImmediate yield（GC + buffer drain）
 */

import { existsSync, statSync, openSync, readSync, closeSync } from 'node:fs';
import { isCheckpointEntry, isDeltaEntry, reconstructSegment } from '../server/lib/delta-reconstructor.js';
...
function expandRawRepeatEntry(raw, expander) {
  if (!raw.includes('"_cxvRepeat"') && !/\/(?:backend-api\/)?codex\/models|\/v1\/models/.test(raw)) return raw;
  try {
    const parsed = JSON.parse(raw);
    const expanded = expander.process(parsed);
    return expanded === parsed ? raw : JSON.stringify(expanded);
  } catch {
    return raw;
  }
}
...
export function streamReconstructedEntries(filePath, onSegment, opts = {}) {
  ...
  let dedup = new Map();
  let sentCount = 0;
  const repeatExpander = createRepeatEntryExpander();
  ...
  for (const rawEntry of iterateRawEntries(filePath)) {
    let entry;
    try { entry = repeatExpander.process(JSON.parse(rawEntry)); } catch { continue; }
 ...
 *
 * server 不做 delta reconstruct；仅 Models 重复项做局部 parse/stringify，
 * 其余条目仍保持原始字符串直通，维持低内存峰值。
...
export async function streamRawEntriesAsync(filePath, onRawEntry, opts = {}) {
  ...
  const dedup = new Map();
  const repeatExpander = createRepeatEntryExpander();
  for (const originalRaw of iterateRawEntries(filePath)) {
    const raw = expandRawRepeatEntry(originalRaw, repeatExpander);
    if (onScan) onScan(raw);
    const key = extractDedupKey(raw);
    if (key) {
      setLatestMapValue(dedup, key, raw);
    } else {
