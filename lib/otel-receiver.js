 ...
 * 解析 spans 并转换为 cx-viewer entry，由共享 V2 写入链路持久化。
 *
...
function spanToEntry(span, resourceAttrs) {
  ...
  const otelSessionId = resourceAttrs['cx-viewer.session']
    || resourceAttrs['service.instance.id']
    || resourceAttrs['service.instance_id']
    || null;
  ...
  for (const evt of events.filter(e => e.name === 'codex.api_request')) {
    ...
    results.push({
      ...
      ...(otelSessionId ? { _otelSessionId: otelSessionId } : {}),
    });
  }
  ...
  for (const evt of events.filter(e => e.name === 'codex.user_prompt')) {
    ...
    results.push({
      ...
      body: {
        model,
        input: [{ role: 'user', content: a['prompt'] || '' }],
        _promptLength: parseInt(a['prompt_length']) || 0,
      },
      response: null,
      duration: 0,
      isStream: false,
      // This event mirrors text already present in the real Responses request.
      // Keep it as supplemental telemetry, never as a second conversation turn.
      mainAgent: false,
      subAgent: false,
      _otelSource: true,
      _otelTraceId: span.traceId,
