...
export function getResponseToolDeclaration(body) {
  if (Array.isArray(body?.tools) && body.tools.length > 0) {
    return { declared: true, tools: body.tools };
  }
  const input = Array.isArray(body?.input) ? body.input : [];
  const item = input.find(candidate => candidate?.type === 'additional_tools'
    && Array.isArray(candidate.tools));
  if (item) return { declared: true, tools: item.tools };
  if (Array.isArray(body?.tools)) return { declared: true, tools: body.tools };
  return { declared: false, tools: [] };
}

export function getResponseTools(body) {
  return getResponseToolDeclaration(body).tools;
}
