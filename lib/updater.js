...
const CODEX_SETTINGS_FILE = join(homedir(), '.codex', 'settings.json');
...
function isAutoUpdateEnabled() {
  ...
  try {
    if (!existsSync(CODEX_SETTINGS_FILE)) return true; // 默认启用
    const settings = JSON.parse(readFileSync(CODEX_SETTINGS_FILE, 'utf-8'));
