  ...
  appendLocked(entry, identity, options) {
    ...
    if (this.summaryMode === 'live') {
      try {
        ...
        if (threadId === this.rootThreadId && timelineRecord.agentRole === 'main') {
          summary.rootMainEvents = (summary.rootMainEvents || 0) + 1;
          const activity = timelineRecord.timestamp || timelineRecord.committedAt;
          if (activity && (!summary.lastRootMainActivity || activity > summary.lastRootMainActivity)) {
            summary.lastRootMainActivity = activity;
          }
        }
