...
import { hashStorageId, sessionArchiveRelativePath, threadStoreToken } from './identity.js';
import {
...
function findSessionArchive(projectDir, sessionId) {
  const target = `${hashStorageId(sessionId, 's_')}.cxvsession`;
  const sessionsDir = join(projectDir, 'sessions');
  for (const year of listDirectories(sessionsDir)) {
    for (const month of listDirectories(year)) {
      for (const day of listDirectories(month)) {
        const candidate = join(day, target);
...
function resolveReferencedRecord(sessionDir, ref, kind) {
  ...
  return readJsonReferenceSync(kind === 'input' ? files.input : files.entries, ref);
}
...
export class LogV2Writer {
  ...
  constructor({
    ...
    this.projectToken = hashStorageId(`${projectId}\u0000${canonicalCwd}`, 'p_');
    this.projectDir = join(rootDir, 'v2', 'projects', this.projectToken);
    ...
    const recovered = withFileLockSync(this.sessionLockPath, () => recoverSessionState(this.sessionDir));
    ...
    if (this.summaryMode === 'live') {
      ...
      withFileLockSync(this.sessionLockPath, () => this.loadOrRebuildSummaryLocked());
    }
  }
