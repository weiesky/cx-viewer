...
const MAX_JSONL_REFERENCE_BYTES = 64 * 1024 * 1024;
const LOCK_WAIT_ARRAY = new Int32Array(new SharedArrayBuffer(4));

export function fsyncDirectorySync(directoryPath) {
  let fd = null;
  try {
    fd = openSync(directoryPath, 'r');
    fsyncSync(fd);
  } catch (error) {
    // Windows does not provide POSIX directory fsync semantics. Keep file
    // fsync strict, but do not make durable mode unusable solely because the
    // platform rejects opening/syncing a directory handle.
    if (process.platform !== 'win32'
        || !['EACCES', 'EPERM', 'EISDIR', 'EINVAL', 'ENOTSUP'].includes(error?.code)) throw error;
  } finally {
    if (fd !== null) closeSync(fd);
  }
}

function assertNoSymlinkComponents(rootDir, filePath) {
  if (!rootDir) return;
  const rel = relative(rootDir, filePath);
  if (!rel || rel === '..' || rel.startsWith(`..${sep}`) || rel.startsWith(sep)) {
    throw new Error('V2 file escapes its archive');
  }
  let current = rootDir;
  ...
  for (let index = 0; index < parts.length - 1; index++) {
    ...
    if (stat.isSymbolicLink() || !stat.isDirectory()) throw new Error(`unsafe V2 path component: ${current}`);
  }
}

function openRegularFileNoFollow(filePath, rootDir = null) {
  assertNoSymlinkComponents(rootDir, filePath);
  const before = lstatSync(filePath);
  if (before.isSymbolicLink() || !before.isFile()) throw new Error(`unsafe V2 file: ${filePath}`);
  const flags = fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW || 0);
  const fd = openSync(filePath, flags);
  try {
    const after = fstatSync(fd);
    if (!after.isFile() || after.dev !== before.dev || after.ino !== before.ino) {
      throw new Error(`V2 file changed while opening: ${filePath}`);
    }
    return fd;
  } catch (error) {
    closeSync(fd);
...
function readRegularFileSync(filePath, encoding, rootDir = null) {
  const fd = openRegularFileNoFollow(filePath, rootDir);
  try {
...
function staleLockByStateAndAge(lockPath, staleMs) {
  const ownerState = lockOwnerState(lockPath);
  if (ownerState === 'gone') return true;
  if (ownerState !== 'unknown') return false;
...
function reclaimStaleLock(lockPath, staleMs) {
  ...
  try {
    ...
    writeSync(reclaimFd, JSON.stringify({ pid: process.pid, createdAt: new Date().toISOString() }));
    closeSync(reclaimFd);
    reclaimFd = undefined;
  } catch (error) {
  ...
  try {
    // Every new acquirer observes the reclaim pathname before creating the
    // owner file. Recheck under that guard so a released stale lock cannot be
    // replaced by a live owner between our diagnosis and unlink.
    if (!existsSync(lockPath)) return true;
    if (!staleLockByStateAndAge(lockPath, staleMs)) return false;
    unlinkSync(lockPath);
    return true;
  } finally {
...
export function withFileLockSync(lockPath, operation, { timeoutMs = 30_000, staleMs = 120_000 } = {}) {
  ...
  while (fd === undefined) {
    if (existsSync(`${lockPath}.reclaim`)) {
      try {
        if (staleLockByStateAndAge(`${lockPath}.reclaim`, staleMs)) {
    ...
    try {
      ...
      let stale = false;
      try { stale = staleLockByStateAndAge(lockPath, staleMs); }
      catch (statError) {
...
export function atomicWriteJsonSync(filePath, value, { durable = true } = {}) {
  ...
  try {
    ...
    if (durable) fsyncDirectorySync(dirname(filePath));
  } finally {
    if (existsSync(tmp)) {
...
export function appendJsonLineSync(filePath, value, { durable = true } = {}) {
  ...
  try {
    ...
    if (durable) {
      fsyncSync(fd);
      if (!existed) fsyncDirectorySync(dirname(filePath));
    }
...
export function readJsonReferenceSync(filePath, ref, { rootDir = null } = {}) {
  ...
  try {
    ...
    // V2 thread records contain references and metadata, never bulk payloads.
    // Bound before allocation so a corrupt archive cannot request a huge Buffer.
    if (ref.length > MAX_JSONL_REFERENCE_BYTES) throw new Error('JSONL reference is too large');
    if (ref.offset > fileSize || ref.length > fileSize - ref.offset) {
      throw new Error('truncated JSONL reference');
    }
...
export function readContentObjectSync(sessionDir, ref) {
  ...
  const raw = readRegularFileSync(join(sessionDir, expected), 'utf8', sessionDir).trimEnd();
