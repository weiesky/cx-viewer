...
export function loadLogV2RuntimeConfigDocument(logDir) {
  ...
  const allowed = new Set([
    'kind', 'version', 'updatedAt', 'writeMode', 'readMode', 'minFreeBytes',
    'minFreePercent', 'failureLimit', 'gateFile', 'projectV1',
  ]);
...
function parseCli(argv) {
  ...
  for (const arg of argv) {
    if (!arg.startsWith('--')) positional.push(arg);
      ...
      if (key === 'write') values.writeMode = value;
      else if (key === 'read') values.readMode = value;
      else if (key === 'min-free-bytes') values.minFreeBytes = value;
      else if (key === 'min-free-percent') values.minFreePercent = value;
      else if (key === 'failure-limit') values.failureLimit = value;
      else if (key === 'gate-file') values.gateFile = value;
      else if (key === 'project-v1') values.projectV1 = value;
      else throw new TypeError(`unknown option --${key}`);
    }
  }
  if (positional.length !== 1) {
    throw new TypeError('Usage: node lib/log-v2/runtime-config.js <log-dir> --write=v1|dual|v2 --read=v1|v2 [--gate-file=PATH] [--project-v1=BOOL]');
  }
