...
import { createV2WireLiveReader } from './materializer.js';
import {
...
function waitForIdle(source) {
  if (!source.running) return Promise.resolve();
  return new Promise(resolve => source.idleWaiters.push(resolve));
}
...
async function pump(source) {
  ...
  source.running = true;
  try {
    const result = await source.reader.read(source.cursor);
    if (source.closed) return;
    ...
    for (const resolve of source.idleWaiters.splice(0)) resolve();
    if (source.pending) { source.pending = false; queueMicrotask(() => { void pump(source); }); }
  }
}
...
function createPublisher({ logDir, file, timelinePath, cursor, seedCheckpoint = null }) {
  ...
  if (cursor.throughSeq === 0 && cursor.timelineBytes === 0) {
    checkpoint = checkpointWireArchiveState(createWireArchiveState(cursor.archive));
  ...
  const source = {
    key, logDir, file, timelinePath, cursor,
    reader: createV2WireLiveReader({ logDir, file, timelinePath, checkpoint }),
    ring: [], ringBytes: 0, subscribers: new Set(),
    running: false, pending: false, closed: false, timer: null, closeTimer: null,
    listener: null, idleWaiters: [],
  };
  ...
  queueMicrotask(() => { void pump(source); });
  return source;
}
...
export function watchV2Timeline({ logDir, file, timelinePath, cursor, seedCheckpoint = null, onCommits, onError }) {
  ...
  const finishReplay = () => {
    if (subscriber.closed) return;
    // Registration and replay are one synchronous critical section. A
    // snapshot-ahead subscriber must not receive the catch-up suffix before
    // its own cursor has become the publisher baseline.
  ...
  if (source.cursor.throughSeq < cursor.throughSeq) {
    (async () => {
      while (!source.closed && source.cursor.throughSeq < cursor.throughSeq) {
        await pump(source);
        await waitForIdle(source);
      }
      finishReplay();
    })().catch(async error => {
      if (!subscriber.closed) await onError(error);
      closeSubscriber(source, subscriber);
    });
  } else {
    finishReplay();
