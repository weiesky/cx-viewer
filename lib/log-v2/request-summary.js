...
const ROOT_FIELDS = Object.freeze([
  'timestamp', 'url', 'proxyUrl', 'proxyProfile', 'proxyWireApi', 'method', 'duration', 'mainAgent', 'subAgent',
  'subAgentName', 'teammate', 'teammateName', 'parentThreadId', 'threadId',
  'project', 'inProgress', 'isCountTokens', '_sdkSource', '_otelSource', '_appServerSource',
...
function bodyHints(body) {
  ...
  const legacyMetadata = body.metadata && typeof body.metadata === 'object' && !Array.isArray(body.metadata)
    ? body.metadata
    : null;
  const metadata = nativeMetadata || legacyMetadata
    ? pick({ ...(legacyMetadata || {}), ...(nativeMetadata || {}) }, ['thread_id', 'turn_id', 'session_id', 'conversation_id', 'user_id', 'parent_thread_id', 'agent_role'])
    : null;
  return {
...
export function selectUsageHeaders(headers) {
...
export function buildRequestSummary(entry, timeline = {}) {
  ...
  const headers = selectUsageHeaders(response?.headers);
...
export function validateRequestSummary(value) {
  ...
  const responseHeaders = value.response?.headers;
  if (responseHeaders !== undefined) {
    const selected = selectUsageHeaders(responseHeaders);
    const originalEntries = responseHeaders && typeof responseHeaders === 'object' && !Array.isArray(responseHeaders)
      ? Object.entries(responseHeaders)
      : [];
    const selectedEntries = selected ? Object.entries(selected) : [];
    const isExactUsageSubset = originalEntries.length === selectedEntries.length
      && originalEntries.every(([name, headerValue]) => name === name.toLowerCase()
        && selected?.[name] === headerValue);
    if (!isExactUsageSubset) errors.push('response.headers must contain only normalized usage headers');
  }
