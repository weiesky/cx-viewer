...
import { readCommittedImportReceipt } from './materializer.js';
...
function readThreadRecord(sessionDir, ref, fileName) {
  ...
  return readJsonReferenceSync(join(sessionDir, 'threads', ref.thread, fileName), ref);
}
...
export function inspectSessionArchive(sessionDir) {
  ...
  if (!existsSync(manifestPath)) errors.push('manifest.json is missing');
    ...
    try {
      ...
      else importReceipt = readCommittedImportReceipt(resolve(sessionDir), manifest);
    } catch (error) {
  ...
  if (importReceipt && importReceipt.entryCount !== previousSeq) {
    errors.push(`legacy import receipt entry count mismatch (${importReceipt.entryCount} receipt vs ${previousSeq} timeline)`);
  }
