...
function threadRecord(sessionDir, ref, name) {
  ...
  return readJsonReferenceSync(join(sessionDir, 'threads', ref.thread, name), ref);
}
...
export function rebuildRequestSummary(sessionDir, descriptor) {
  ...
  const entry = {
    ...
    ...(response !== undefined || responseBody !== undefined
      ? { response: { ...(response || {}), ...(responseBody !== undefined ? { body: responseBody } : {}) } }
      : {}),
  };
