...
import { hashStorageId, threadStoreToken } from './identity.js';
import {
...
const PROJECT_TOKEN_PATTERN = /^p_[a-f0-9]{64}$/;
const THREAD_TOKEN_PATTERN = /^t_[a-f0-9]{64}$/;
const SESSION_DIR_PATTERN = /^s_[a-f0-9]{64}\.cxvsession$/;
...
function readThreadRecord(sessionDir, ref, fileName) {
  return readJsonReferenceSync(threadRecordPath(sessionDir, ref, fileName), ref);
}
...
function listSessionDirs(projectDir) {
  ...
  const sessions = [];
  for (const year of listDirectories(sessionsRoot)) {
    for (const month of listDirectories(year)) {
      for (const day of listDirectories(month)) {
        for (const candidate of listDirectories(day)) {
          if (SESSION_DIR_PATTERN.test(basename(candidate))) sessions.push(candidate);
        }
      }
    }
  }
...
export function discoverV2SessionArchives(logDir, { projectId = null } = {}) {
  const archives = [];
  const errors = [];
  const projectsRoot = join(logDir, 'v2', 'projects');
  for (const projectDir of listDirectories(projectsRoot)) {
    const projectToken = basename(projectDir);
    if (!PROJECT_TOKEN_PATTERN.test(projectToken)) {
      errors.push({ scope: 'project', path: relative(logDir, projectDir), error: 'invalid project storage token' });
      continue;
    }
    let project;
    try {
      project = readJson(join(projectDir, 'project.json'));
      const validation = validateProjectManifest(project);
      if (!validation.ok) throw new Error(validation.errors.join('; '));
      if (hashStorageId(`${project.projectId}\u0000${project.canonicalCwd}`, 'p_') !== projectToken) {
        throw new Error('project storage token does not match manifest identity');
      }
    } catch (error) {
...
export function listV2LocalLogs(logDir, currentProjectName = '') {
  ...
  for (const projectDir of listDirectories(projectsRoot)) {
    const projectToken = basename(projectDir);
    if (!PROJECT_TOKEN_PATTERN.test(projectToken)) continue;
    try {
      const project = readJson(join(projectDir, 'project.json'));
      const validation = validateProjectManifest(project);
      if (!validation.ok) continue;
      if (hashStorageId(`${project.projectId}\u0000${project.canonicalCwd}`, 'p_') !== projectToken) continue;
      for (const sessionDir of listSessionDirs(projectDir)) {
        try {
...
export function resolveV2SessionFile(logDir, file) {
  if (typeof file !== 'string' || !file.endsWith('/timeline.jsonl') || file.split('/').includes('..')) {
    throw new TypeError('invalid V2 session file');
  }
  const root = resolve(logDir, 'v2', 'projects');
  const requestedTimelinePath = resolve(logDir, file);
  const requestedSessionDir = dirname(requestedTimelinePath);
  if (!contained(root, requestedTimelinePath) || !SESSION_DIR_PATTERN.test(basename(requestedSessionDir))) {
    const error = new Error('Access denied');
    error.code = 'ACCESS_DENIED';
    throw error;
  }
  if (!existsSync(requestedTimelinePath) || !existsSync(join(requestedSessionDir, 'manifest.json'))) {
  ...
  if (!contained(realRoot, timelinePath) || !contained(realRoot, sessionDir)
      || dirname(timelinePath) !== sessionDir || !SESSION_DIR_PATTERN.test(basename(sessionDir))) {
    const error = new Error('Access denied');
    error.code = 'ACCESS_DENIED';
    throw error;
  }
  const manifest = readJson(join(sessionDir, 'manifest.json'));
  const validation = validateSessionManifest(manifest);
  if (!validation.ok) throw new Error(`invalid V2 session manifest: ${validation.errors.join('; ')}`);
  readCommittedImportReceipt(sessionDir, manifest);
  return { sessionDir, timelinePath };
}

export function isV2SessionFile(file) {
  return typeof file === 'string' && file.startsWith('v2/projects/') && file.endsWith('/timeline.jsonl');
}
...
export function findV2SessionFileByLegacyLog(logDir, legacyLogFile, { conversationOnly = false } = {}) {
  ...
  const projectsRoot = join(logDir, 'v2', 'projects');
  for (const projectDir of listDirectories(projectsRoot)) {
    if (!PROJECT_TOKEN_PATTERN.test(basename(projectDir))) continue;
    for (const sessionDir of listSessionDirs(projectDir)) {
      try {
        const manifest = readJson(join(sessionDir, 'manifest.json'));
        if (!validateSessionManifest(manifest).ok) continue;
...
export function findLatestV2SessionFile(logDir, { projectId, canonicalCwd } = {}) {
  ...
  const projectToken = hashStorageId(`${projectId}\u0000${canonicalCwd}`, 'p_');
  const projectDir = join(logDir, 'v2', 'projects', projectToken);
  ...
  for (const sessionDir of listSessionDirs(projectDir)) {
    try {
      const manifest = readJson(join(sessionDir, 'manifest.json'));
      const validation = validateSessionManifest(manifest);
      if (!validation.ok || manifest.projectId !== projectId) continue;
