...
// Read compatibility for imports that are already valid V2 archives.
...
export function validateSessionSummary(value) {
  ...
  if (value.rootMainEvents !== undefined && !nonNegativeInteger(value.rootMainEvents)) {
    errors.push('rootMainEvents must be a non-negative safe integer when present');
  }
  if (value.rootMainEvents !== undefined) {
    if (value.rootMainEvents > value.committedEvents) {
      errors.push('rootMainEvents must not exceed committedEvents');
    }
    if (value.rootMainEvents === 0 && value.lastRootMainActivity !== null) {
      errors.push('lastRootMainActivity must be null when rootMainEvents is zero');
    }
    if (value.rootMainEvents > 0 && !isoTimestamp(value.lastRootMainActivity)) {
      errors.push('lastRootMainActivity is required when rootMainEvents is positive');
    }
  } else if (value.lastRootMainActivity !== undefined) {
    errors.push('rootMainEvents is required when lastRootMainActivity is present');
  }
  const hasTimelineIdentity = value.timelineFileId !== undefined
    || value.timelineFileVersion !== undefined || value.timelineTailHash !== undefined;
  if (hasTimelineIdentity) {
    if (!nonEmptyString(value.timelineFileId)) errors.push('timelineFileId is required for timeline identity');
    if (!nonEmptyString(value.timelineFileVersion)) errors.push('timelineFileVersion is required for timeline identity');
    if (!sha256Hex(value.timelineTailHash)) errors.push('timelineTailHash must be a SHA-256 hex digest');
  }
