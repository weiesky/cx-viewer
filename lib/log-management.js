import { readFileSync, writeFileSync, existsSync, statSync, readdirSync, unlinkSync, realpathSync, renameSync, appendFileSync, openSync, readSync, closeSync } from 'node:fs';
...
import { createModelCatalogLogCompactor } from './model-catalog-log.js';
...
export function mergeLogFiles(logDir, files) {
  ...
  const modelCatalogCompactor = createModelCatalogLogCompactor();
  for (const f of files) {
    ...
    streamReconstructedEntries(filePath, (segment) => {
      ...
      for (const entry of segment) {
        ...
        delete entry._inputDigest;
        delete entry._baseInputDigest;
        delete entry._baseMessageCount;
        const storedEntry = modelCatalogCompactor.process(entry);
        if (storedEntry) chunk += JSON.stringify(storedEntry) + '\n---\n';
      }
