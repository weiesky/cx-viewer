import { readFileSync, existsSync, statSync, readdirSync, realpathSync, unlinkSync } from 'node:fs';
import { basename, dirname, join, relative, sep } from 'node:path';
import { deleteV2SessionFile, isV2SessionFile, listV2LocalLogs, readV2LogEntries, resolveV2SessionFile } from './log-v2/materializer.js';
 ...
 */
function removeUnreferencedRawSidecars(logDir, removedContexts) {
  if (removedContexts.length === 0) return;
  const remaining = [];
  const grouped = listV2LocalLogs(logDir);
  for (const logs of Object.values(grouped)) {
    if (!Array.isArray(logs)) continue;
    for (const log of logs) {
      try { remaining.push(rawContextForV2(logDir, log.file)); } catch {}
    }
  }
  for (const { refs, rawDir } of removedContexts) {
    for (const ref of refs) {
      const stillReferenced = remaining.some(context => context.rawDir === rawDir
        && context.refs.some(candidate => candidate.streamId === ref.streamId && candidate.sidecar === ref.sidecar));
      if (!stillReferenced) {
        for (const segment of rawSegmentFiles(rawDir, ref.sidecar)) {
          try { unlinkSync(segment.path); } catch {}
        }
      }
    }
  }
...
export function deleteLogFiles(logDir, files, { protectedFiles = [] } = {}) {
  ...
  for (const file of files) {
    ...
    try {
      let rawContext = null;
      try { rawContext = rawContextForV2(logDir, file); } catch {}
      deleteV2SessionFile(logDir, file);
      if (rawContext) removedContexts.push(rawContext);
      results.push({ file, ok: true });
    } catch (err) {
      results.push({ file, error: err.message });
    }
  }
  removeUnreferencedRawSidecars(logDir, removedContexts);
  return results;
}
