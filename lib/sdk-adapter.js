...
export function sdkUsageToViewerUsage(usage) {
  ...
  const hasCached = usage.cached_input_tokens != null || usage.cachedInputTokens != null
    || usage.cache_write_input_tokens != null || usage.cacheWriteInputTokens != null
    || inputDetails?.cached_tokens != null || inputDetails?.cachedTokens != null
    || inputDetails?.cache_write_tokens != null || inputDetails?.cacheWriteTokens != null;
  const cached = usage.cached_input_tokens ?? usage.cachedInputTokens
    ?? inputDetails?.cached_tokens ?? inputDetails?.cachedTokens ?? 0;
  const cacheWrite = usage.cache_write_input_tokens ?? usage.cacheWriteInputTokens
    ?? inputDetails?.cache_write_tokens ?? inputDetails?.cacheWriteTokens ?? 0;
  ...
  return {
    ...
    total_tokens: total,
    ...(hasCached ? { input_tokens_details: { cached_tokens: cached, cache_write_tokens: cacheWrite } } : {}),
  };
}
...
export function sdkToJSONLEntry(assistantMsg, messages, model, projectName, opts = {}) {
  ...
  const entry = {
    ...
    body: {
      ...
      instructions: opts.systemPrompt || 'You are Codex',
      tools: opts.tools || DEFAULT_CODEX_TOOLS,
      input: cloneJson(messages || []),
      metadata: {
