...
let _zlibLimitWarningShown = false;
export function warnIfZlibOutputLimitUnsupported() {
  if (_zlibLimitWarningShown) return;
  _zlibLimitWarningShown = true;
  const [major, minor] = process.versions.node.split('.').map(Number);
  if (major < 20 || (major === 20 && minor < 12)) {
    console.warn('[CX-Viewer] This Node.js version predates zlib maxOutputLength (Node >=20.12.0 required); decompression size limits are not enforced.');
  }
}
...
const MAX_DECODED_REQUEST_BODY_BYTES = 32 * 1024 * 1024;
...
export function inferCodexWorkspaceCwd(body) {
  ...
  const paths = Object.keys(workspaces).filter(value => isAbsolute(value));
...
function decodeCompressedRequestBody(buffer, encoding) {
  ...
  for (let i = enc.length - 1; i >= 0; i--) {
    ...
    if (e === 'gzip' || e === 'x-gzip') {
      out = zlib.gunzipSync(out, { maxOutputLength: MAX_DECODED_REQUEST_BODY_BYTES });
    } else if (e === 'br') {
      out = zlib.brotliDecompressSync(out, { maxOutputLength: MAX_DECODED_REQUEST_BODY_BYTES });
    } else if (e === 'deflate') {
      try {
        out = zlib.inflateSync(out, { maxOutputLength: MAX_DECODED_REQUEST_BODY_BYTES });
      } catch {
        out = zlib.inflateRawSync(out, { maxOutputLength: MAX_DECODED_REQUEST_BODY_BYTES });
      }
    } else if (e === 'zstd') {
      if (typeof zlib.zstdDecompressSync !== 'function') {
        throw new Error('zstd decompression is not supported by this Node.js runtime');
      }
      out = zlib.zstdDecompressSync(out, { maxOutputLength: MAX_DECODED_REQUEST_BODY_BYTES });
    } else {
      throw new Error(`unsupported content-encoding: ${e}`);
    }
    if (out.length > MAX_DECODED_REQUEST_BODY_BYTES) throw new Error('decoded request body is too large');
  }
  return out;
}
