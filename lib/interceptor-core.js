    ...
    for (let i = parts.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        if (entry.mainAgent) {
          ...
          const msgCount = entry._totalMessageCount || (Array.isArray(input) ? input.length : 0);
          if (msgCount === 1) {
            originIndex = i;
            break;
          }
 ...
 */
export function rotateLogFile(currentFile, newFile, maxSize, nextWriteBytes = 0) {
  try {
    if (!existsSync(currentFile)) return { rotated: false };
    const size = statSync(currentFile).size;
    // Rotate before the next record is appended. Keep an empty file writable
    // even when a single oversized record exceeds maxSize; it cannot be split
    // without corrupting the JSONL record boundary.
    if (size === 0 || size + Math.max(0, nextWriteBytes) < maxSize) return { rotated: false };
    if (!newFile || newFile === currentFile) return { rotated: false };
