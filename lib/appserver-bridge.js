 ...
 *  Honors CXV_ORIGINAL_* (set when the PTY proxy redirect is active) so the
 *  display url on reconstructed entries shows the true upstream, not the
 *  loopback proxy that Codex was redirected to. */
function buildApiUrl() {
  const base = process.env.CXV_ORIGINAL_BASE_URL
    || getOriginalCodexBaseUrl()
    || DEFAULT_API_BASE;
...
function appendUserMessage(state, content, clientId) {
  if (!state) return;
...
function buildFullRequestBody(state = getThreadState(), threadId = state.threadId) {
  ...
  return {
    ...
    system: systemPrompt,
    messages: state.messages.map(m => cloneJson(m)),
...
function handleCompletedItem(item, state, base) {
  ...
  if (type === 'userMessage') {
    appendUserMessage(state, item.content || [], item.clientId || item.id);
    return;
  }
  ...
  if (type === 'plan') {
    ...
    upsertToolUse(state, item.id || `codex-plan:${state.turnId || state.threadId || 'turn'}`, 'ExitPlanMode', buildCodexPlanItemInput({ ...item, text }));
    return;
  }
...
function parseServerMessage(msg) {
  ...
  // For transcript purposes, map Codex request_user_input onto the existing
  // AskUserQuestion tool_use/tool_result shape consumed by ChatView.
  if (msg.id !== undefined && msg.id !== null) {
    if (method === 'item/tool/requestUserInput') {
      ...
      upsertToolUse(state, toolId, 'AskUserQuestion', {
        questions,
  ...
  if (method === 'turn/plan/updated') {
    ...
    upsertToolUse(state, state.turnPlanToolId, 'ExitPlanMode', buildCodexTurnPlanInput(state.turnPlan));
    return;
  }
...
export async function startAppServerBridge(options) {
  const { cwd, codexPath, logFile, env = process.env, extraConfigArgs = [] } = options;
  _logFile = logFile;
  _currentModel = null;
