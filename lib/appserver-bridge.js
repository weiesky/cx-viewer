 ...
 * 并转换为 cx-viewer entry 格式写入 LOG_FILE。原始 JSON-RPC 帧不再重复嵌入
...
function writeEntry(entry) {
  ...
  try {
    ...
    else appendFileSync(_logFile, JSON.stringify(storedEntry) + '\n---\n');
    if (rawRef) {
      if (result?.completion && typeof result.completion.then === 'function') {
        void Promise.resolve(result.completion).then((completion) => {
          ...
          || result.durable === true
          || (result.accepted !== true && result.written === true)) {
        // Undefined is the historical callback contract; written:true remains
        // the synchronous writer contract used by V1/dual mode.
