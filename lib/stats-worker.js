...
export function parseJsonlFile(filePath) {
  ...
  try {
    ...
    if (!content.trim()) return { models, summary: { requestCount: 0, sessionCount: 0, turnCount: 0, input_tokens: 0, output_tokens: 0, cache_read_tokens: 0, cache_write_tokens: 0 }, preview: [] };
    ...
    for (const raw of entries) {
      try {
        ...
        if (entry.mainAgent && !isOpenAiResponsesMasterEntry(entry) && Array.isArray(entry.body?.input)) {
          const msgs = entry.body.input;
          // Delta storage: 使用 _totalMessageCount（delta 条目）或 input.length（全量条目）
          const len = entry._totalMessageCount || msgs.length;
          if (!isSuggestionMode(msgs)) {
            // input 数量大幅缩减：新会话（/clear 等），重置追踪
        ...
        if (!models[model]) {
          models[model] = { count: 0, input_tokens: 0, output_tokens: 0, cache_read_tokens: 0, cache_write_tokens: 0 };
        }
        models[model].count++;
        ...
        if (usage) {
          ...
          models[model].cache_read_tokens += cache.read;
          models[model].cache_write_tokens += cache.write;

          totalInput += inp;
          totalOutput += out;
          totalCacheRead += cache.read;
          totalCacheWrite += cache.write;
        }
      } catch {
        // 跳过无法解析的条目
      }
...
function generateProjectStats(projectDir, projectName, onlyFile) {
  ...
  for (const f of Object.values(filesStats)) {
    ...
    totalCacheRead += f.summary.cache_read_tokens || 0;
    totalCacheWrite += f.summary.cache_write_tokens || 0;
  }
