...
export function scan(logDir, onlyProject = null) {
  const grouped = listV2LocalLogs(logDir, onlyProject);
  const projects = onlyProject
    ? (grouped[onlyProject] ? [onlyProject] : [])
    : Object.keys(grouped).filter(project => project !== '_currentProject');
  mkdirSync(join(logDir, 'v2-stats'), { recursive: true });
  if (onlyProject && !projects.includes(onlyProject)) {
    try { unlinkSync(cachePath(logDir, onlyProject)); } catch {}
  }
...
function scheduleDrain() {
  ...
  setImmediate(() => {
    ...
    const scanAll = pendingScanAll;
    pendingScanAll = null;
    const updates = [...pendingUpdates.values()];
    pendingUpdates.clear();
    try {
      if (scanAll && existsSync(scanAll)) {
        scan(scanAll);
...
parentPort?.on('message', ({ type, logDir, projectName }) => {
  if (type === 'scan-all') pendingScanAll = logDir;
  else if ((type === 'init' || type === 'update') && projectName) {
